import { CanvasTableColumn } from "./CustomCanvasTable";
export class CanvasTableEdit {
    private readonly column: CanvasTableColumn;
    private readonly row: number;
    private readonly inputeElement: HTMLInputElement;
    private readonly onRemove:(cancel:boolean, newData:string)=>void;

    constructor (col:CanvasTableColumn, row: number, data:string, cellHeight: number, onRemve:(cancel:boolean, newData:string)=>void) {
        this.column = col;
        this.row = row;
    ...
    public updateEditLocation(r:number, posX:number, posY: number, offsetTop:number, offsetLeft:number, cellHeight: number){
